...
exports.index = async (req, res) => {
  ...
  res.render("products/index", {
    pageTitle: "NewEgg.ca Scrapper Site",
    products,
  });
};
...
exports.update = async (req, res) => {
  const url =
    "https://www.newegg.ca/Xbox-One-Video-Games/SubCategory/ID-3218?cm_sp=Cat-XboxOne_1-_-Button-_-Games_2";
...
async function scrapeIt(url) {
  ...
  await page.waitForSelector(`[class^="list-wrap"]`, {
    visible: true,
    timeout: 120,
  });
  ...
  const content = await page.evaluate(async () => {
    const productScrape = document.querySelectorAll(".item-cell");
    const products = [];
    ...
    for (let product of productScrape) {
      ...
      const title = product.querySelector(`[class^="item-title"]`).textContent;
      const price = product.querySelector(`[class^="price"]`).textContent;
